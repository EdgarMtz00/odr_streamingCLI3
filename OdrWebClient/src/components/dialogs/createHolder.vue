...
<script>
  export default {
    ...
    methods: {
      ...
      loadCharacters () {
        let urlBase = this.$store.getters.urlBase
        this.axios.post(urlBase + 'connections/streamingContent/creating/getAllCharacters.php').then(response => {
              console.log(response)
              response.data.forEach(element => {
                this.characters.push({
                  thumbnail: urlBase + 'Characters/' + element.URLPersonaje + "/profile.jpg",
